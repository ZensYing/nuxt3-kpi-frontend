...
const fetchAdvertisementData = async () => {
  ...
  try {
    ...
    const adData = await useBcApi<IResponse<Advertisement[]>>(
      `/items/advertisements?filter[status][_eq]=published&filter[slug][_eq]=${slug}&fields=name,file,slug,advertisement_type.type,mobile_only,home_page,detail_page`,
      { method: 'GET' }
    );
...
const downloadPDF = async () => {
  ...
  try {
   ...
   const pdfUrl = 'https://reports.business-cambodia.com/pdf';
    if (!pdfUrl) {
