<template>
  <div class="container mx-auto px-4 py-8 max-w-6xl">
    ...
    <div v-if="loading" class="flex justify-center items-center h-64">
      <div
        class="inline-block h-12 w-12 animate-spin rounded-full border-4 border-blue-600 border-r-transparent align-[-0.125em]"
        role="status">
    ...
    <div v-else class="bg-white rounded-lg shadow-sm">
      ...
      <div class="flex flex-col md:flex-row justify-between items-center p-4 border-b">
        <div class="mb-4 md:mb-0">
          <div class="relative">
            <input v-model="searchQuery" type="text" placeholder="Search advertisements..."
      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="advertisement in paginatedAdvertisements" :key="advertisement.slug"
              class="hover:bg-gray-50 transition-colors">
              <td class="py-4 px-4 text-sm md:text-base text-gray-800">{{ advertisement.name }}</td>
              <td class="py-4 px-4 text-center">
                <div class="flex justify-center">
                  <img :src="getImageUrl(advertisement.file)" :alt="`${advertisement.name} image`"
                    class="w-12 h-12 md:w-16 md:h-16 object-cover rounded-lg bg-gray-100" loading="lazy" width="64"
                    height="64" />
                </div>
              </td>
              <td class="py-4 px-4 text-center">
                <NuxtLink :to="`/dashboard/ads/report-tc/${advertisement.slug}`"
      ...
      <div class="flex flex-col md:flex-row justify-between items-center p-4 border-t">
        ...
        <div class="flex items-center space-x-2">
          <button @click="goToPage(1)" :disabled="currentPage === 1" :class="paginationButtonClass(currentPage === 1)"
            aria-label="First page">
            First
          </button>
          <button @click="prevPage" :disabled="currentPage === 1" :class="paginationButtonClass(currentPage === 1)"
          ...
          <button @click="nextPage" :disabled="currentPage === totalPages"
            :class="paginationButtonClass(currentPage === totalPages)" aria-label="Next page">
            Next
          </button>
          <button @click="goToPage(totalPages)" :disabled="currentPage === totalPages"
            :class="paginationButtonClass(currentPage === totalPages)" aria-label="Last page">
            Last
          </button>
        </div>
      </div>
...
// Constants
const API_BASE_URL_BC = 'https://tech-cambodia.com/cms';
...
const fetchAdvertisements = async () => {
  loading.value = true;
  try {
    const response = await $fetch<AdvertisementsResponse>(`${API_BASE_URL_BC}/items/advertisement`, {
      params: {
